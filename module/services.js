...
export const fetchSecureConfig = async () => {
    ...
    try {
        ...
        if (data) {
            ...
            state.devKey = data.dev_key; // New Dev Key fetch
        }
    } catch (e) {
...
export const setupRealtimeListener = () => {
    ...
    channel
        ...
        // 7. ENTERPRISE APPOINTMENTS
        .on('postgres_changes', { event: '*', schema: 'public', table: 'enterprise_appointments' }, async () => {
            if (state.activeHubPanel === 'enterprise') {
                // If managing, update manager view
                if(state.activeEnterpriseManagement) {
                    await fetchEnterpriseDetails(state.activeEnterpriseManagement.id);
                }
                // If viewing my appointments, update client view
                if(state.activeCharacter) {
                    await fetchClientAppointments(state.activeCharacter.id);
                }
                render();
            }
        })
...
// ... (fetchDiscordWidgetData, fetchPublicLandingData, etc. remain same) ...
...
// ... (Skipping large chunks of unchanged services) ...
...
// ... (Emergency Calls, Reports, Stats, etc.) ...
...
export const fetchEnterpriseDetails = async (entId) => {
    ...
    // --- AUTO-ECOLE SEEDING (Manager View) ---
    ...
    // Need to fetch profiles for Discord names
    ...
    if (appointments && appointments.length > 0) {
        ...
        enhancedAppointments = appointments.map(a => {
            ...
            return {
                ...a,
                discord_username: profile ? profile.username : 'Inconnu'
            };
        });
    }
...
// ... (Rest of file including advent calendar etc. remains same) ...
export const claimAdventReward = async (targetDay) => {
    ...
    // --- FIX RETROACTIVE FOR DAY 16 ---
    // If the user has claimed Day 16 but missed the item due to bug, give it now
    if (targetDay === 16 && claimedDays.includes(16)) {
        ...
        if (!hasItem) {
             await state.supabase.from('inventory').insert({
                character_id: charId,
                name: "Anniversaire de la patate 2025",
                quantity: 1,
    ...
    // NEW: SPECIAL ITEM ON DEC 16
    if (targetDay === 16) {
        // Safe Insert (removed is_virtual which caused error)
        try {
            const { error } = await state.supabase.from('inventory').insert({
                character_id: charId,
                name: "Anniversaire de la patate 2025",
                quantity: 1,
                estimated_value: 5000
...
export const fetchDrugLab = async (charId) => {
    ...
    if (!lab) { const { data: newLab } = await state.supabase.from('drug_labs').insert([{ character_id: charId }]).select().single(); lab = newLab; }
    state.drugLab = lab;
};
export const updateDrugLab = async (updates) => { if(!state.activeCharacter) return; await state.supabase.from('drug_labs').update(updates).eq('character_id', state.activeCharacter.id); await fetchDrugLab(state.activeCharacter.id); };
