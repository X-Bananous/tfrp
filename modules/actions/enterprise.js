...
export const addItemToMarket = async (e) => {
    ...
    try {
        ...
        if (taxPaid) {
            await state.supabase.from('enterprises')
...
export const checkPromoCode = async (code, entId, basePrice, qty) => {
    ...
    if (promo) {
        ...
        if (new Date(promo.expires_at) > now && promo.current_uses < promo.max_uses) {
            ...
            const total = Math.ceil(priceHT * 1.20);
...
export const openBuyModal = (itemId) => {
    ...
    const priceTTC = Math.ceil(priceHT * 1.20); 
    let maxAffordable = 0;
    ...
    ui.showModal({
        ...
        content: `
            ...
            <div class="bg-white/5 p-4 rounded-xl border border-white/5 space-y-3">
                ...
                <div class="flex justify-between items-center text-sm"><span class="text-gray-400">TVA (20%)</span><span class="font-mono text-gray-500">+ $${Math.ceil(priceHT * 0.20).toLocaleString()}</span></div>
                ...
                <div class="pt-3 border-t border-white/5"><label class="text-xs text-gray-500 uppercase font-bold mb-1 block">${isService ? 'Nombre de sessions / places' : 'Quantité souhaitée'}</label><input type="number" id="buy-qty" class="glass-input w-full p-2 rounded-lg text-sm font-mono" value="1" min="1" max="${item.quantity}" oninput="window.updateBuyTotal(${priceHT})"></div>
                <div class="pt-2">
                ...
                <div class="bg-black/40 p-3 rounded-lg mt-2 space-y-1">
                    ...
                    <div class="flex justify-between items-center text-xs text-gray-500"><span>Total TVA</span><span id="buy-vat">$${Math.ceil(priceHT * 0.20).toLocaleString()}</span></div>
...
export const confirmBuyItem = async (itemId, quantity, promoCode) => {
    // On re-récupère l'item depuis le marché pour avoir l'enterprise_id à jour
    const item = state.enterpriseMarket.find(i => i.id === itemId);
    if(!item) return;
    ...
    if(entData) await state.supabase.from('enterprises').update({ balance: (entData.balance || 0) + priceHT }).eq('id', entId);
    ...
    // CRÉATION DE LA FACTURE (Avec entreprise_id garanti)
    await state.supabase.from('invoices').insert({
        buyer_id: charId,
        enterprise_id: entId, // On utilise l'ID forcé
        item_name: item.name,
        quantity: quantity,
...
export const handleAppointment = async (aptId, action, serviceName, charId) => {
    ...
    if (action === 'reject') {
        ...
        if (lowerService.includes("permis")) {
            ...
            if(!existing) await state.supabase.from('inventory').insert({ character_id: charId, name: serviceName, quantity: 1, estimated_value: 0 });
            ui.showToast("Permis accordé.", "success");
            ...
            await state.supabase.from('characters').update({ driver_license_points: Math.min(12, (char.driver_license_points || 0) + 3) }).eq('id', charId);
            ui.showToast(`Stage validé.`, "success");
        }
        await state.supabase.from('enterprise_appointments').delete().eq('id', aptId);
    }
    await services.fetchEnterpriseDetails(entId);
