...
export const confirmBuyItem = async (itemId, quantity, promoCode) => {
    ...
    // Check if it is Auto-école for specific logic
    ...
    if (isAutoEcole && itemNameLower.includes("permis")) {
    ...
    // 2. CREATE APPOINTMENT IF AUTO-ECOLE
    if (isAutoEcole) {
        // IMPORTANT: Because of generic policy issues in some Supabase configs with anon auth,
        // we assume the "Enable insert for all" policy has been applied by the user.
        ...
        if (apptError) {
            ...
            ui.showToast("Rendez-vous pris. En attente de l'instructeur.", 'success');
        }
    } else {
    ...
    // 4. Update Stock (Skip for Auto Ecole)
    if (!isAutoEcole) {
