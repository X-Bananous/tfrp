...
export const openAdminEditChar = (charId) => {
    const char = state.allCharactersAdmin.find(c => c.id === charId);
    if(char) {
        // Prepare dummy session user to trick the create form, or modify form to support admin mode
        // Cleaner approach: Set a flag
        state.editingCharacter = char;
        state.isAdminEditing = true; // New flag
        router('create');
    }
};

export const openAdminCreateChar = async () => {
    // Show modal to search for target User ID
    ui.showModal({
...
export const selectUserForPerms = async (userId) => {
    ...
    if (state.isAdminEditing) {
        // Update the preview text in modal
