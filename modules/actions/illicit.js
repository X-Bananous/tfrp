...
export const gangDistribute = async (targetId, targetName) => {
    ui.showModal({
        ...
        content: `Combien voulez-vous donner à <b>${targetName}</b> depuis le coffre ?<br><input type="number" id="distrib-amount" class="glass-input w-full p-2 mt-2 rounded" placeholder="Montant">`,
        confirmText: "Envoyer",
        onConfirm: async () => {
...
export const finishHeist = async () => {
    ...
    const message = `Braquage RÉUSSI ! Butin total: $${rawLoot.toLocaleString()}.${gangTax > 0 ? `<br><span class="text-xs text-gray-400">Taxe Gang (25%): -$${gangTax.toLocaleString()}</span>` : ''}<br>Votre part: <span class="text-emerald-400 font-bold">$${myShare.toLocaleString()}</span>.`;
