...
export const setIllicitTab = async (tab) => {
    ...
    try {
        ...
        else if (tab === 'drugs') await services.fetchDrugLab(state.activeCharacter.id);
        else if (tab === 'gangs') {
...
export const searchGangSearch = (role, query) => {
    ...
    // Use specific IDs to avoid re-rendering entire view
    const dropdownId = role === 'Leader' ? 'gang-leader-dropdown' : 'gang-coleader-dropdown';
    const container = document.getElementById(dropdownId);
    
    if(!container) return; // Fallback if view not ready
...
export const selectGangLeader = (role, id, name) => {
    ...
    state.gangCreation.searchResults = [];
    render(); // Re-render to update input values safely
};
...
export const leaveGang = async () => {
    ui.showModal({
        ...
        type: "danger",
        onConfirm: async () => {
            await state.supabase.from('gang_members').delete().eq('character_id', state.activeCharacter.id);
...
export const manageGangRequest = async (charId, action) => {
    ...
    if (action === 'accept') {
        ...
        return; // Early return because modal is async
    }
         ...
         ui.showModal({
             ...
             content: `
                ...
                <div class="relative">
                    ...
                    <input type="text" 
                        id="bounty-winner-input"
                        placeholder="Rechercher le chasseur de primes..." 
                        oninput="actions.searchBountyWinner(this.value)"
                        class="glass-input w-full p-3 pl-10 rounded-xl text-sm" 
                        autocomplete="off">
                ...
                <button id="bounty-confirm-btn" onclick="actions.confirmBountyWinner()" class="mt-4 w-full glass-btn py-3 rounded-xl font-bold opacity-50 cursor-not-allowed" disabled>
                    Valider le Paiement
                </button>
             `,
             cancelText: "Fermer"
...
export const startDrugAction = async (stage, e) => {
    ...
    // FIX: Get FormData from the parent FORM, not the button
    ...
    if (!type || !amount) {
        ui.showToast("Erreur formulaire.", 'error');
        toggleBtnLoading(btn, false);
        return;
    }
...
// DIRECT CALL HANDLER TO AVOID MODAL RACE CONDITION
...
export const createLobby = async (heistId) => {
    ...
    // Helper to prompt for Access Type
    const promptAccessType = (location = null) => {
        // We delay slightly to allow the previous modal to close properly without conflict
        setTimeout(() => {
                ...
                content: `
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <button onclick="actions.finalizeHeistCreation('${heistId}', ${locStr}, 'private')" class="p-4 rounded-xl bg-purple-500/20 border border-purple-500 hover:bg-purple-500/30 transition-colors text-left group">
                            ...
                            <div class="text-[10px] text-gray-400 mt-1 text-center">Sur Dossier</div>
                        </button>
                        <button onclick="actions.finalizeHeistCreation('${heistId}', ${locStr}, 'open')" class="p-4 rounded-xl bg-green-500/20 border border-green-500 hover:bg-green-500/30 transition-colors text-left group">
                            ...
                            <div class="text-[10px] text-gray-400 mt-1 text-center">Accès Libre</div>
                        </button>
                    </div>
                `,
                cancelText: "Annuler",
    ...
    if (heist.requiresLocation) {
        // Show specific modal for address selection
        ui.showModal({
            ...
            onConfirm: () => {
                ...
                // The modal closes automatically after onConfirm, promptAccessType will wait 300ms then open the next one
                promptAccessType(loc);
            }
        });
    } else {
        promptAccessType(null);
    }
};

export const requestJoinLobby = async (lobbyId) => {
    await services.joinLobbyRequest(lobbyId);
    render();
};
...
export const acceptHeistApplicant = async (targetCharId) => {
    ...
    if (currentCount >= heist.teamMax) {
        ui.showToast(`Équipe complète (Max ${heist.teamMax}).`, 'error');
        return;
    }
...
export const startHeistLobby = async (timeSeconds) => {
    ...
    if (currentCount < heist.teamMin) {
        ui.showToast(`Effectif insuffisant (Min ${heist.teamMin}).`, 'error');
        return;
    }
...
export const finishHeist = async () => {
    ...
    // Safety check - but allow manual finish if close enough
    if (lobby.end_time > now) {
         ui.showToast("L'opération est toujours en cours ! Attendez la fin du timer.", 'error');
         return;
    }
    ...
    // Loop through all team members to pay them
    for(const member of members) {
        // Fetch current balance for atomic-like update
    ...
    const message = `Braquage RÉUSSI ! Butin total: $${rawLoot.toLocaleString()}.${gangTax > 0 ? `<br><span class="text-xs text-gray-400">Taxe Gang (25%): -$${gangTax.toLocaleString()}</span>` : ''}<br>Part par personne: <span class="text-emerald-400 font-bold">$${myShare.toLocaleString()}</span>.`;
...
export const stopHeist = async () => {
    ...
    ui.showModal({
        ...
        content: "Voulez-vous vraiment annuler l'opération en cours ? Cela sera considéré comme un échec/abandon.",
        confirmText: "Abandonner",
        type: "danger",
        onConfirm: async () => {
             // If active, mark failed. If setup, just delete.
             if (state.activeHeistLobby.status === 'active') {
                 await state.supabase.from('heist_lobbies').update({ status: 'failed' }).eq('id', state.activeHeistLobby.id);
             } else {
                 await state.supabase.from('heist_lobbies').delete().eq('id', state.activeHeistLobby.id);
             }
             state.activeHeistLobby = null;
             state.heistMembers = [];
             ui.showToast("Opération annulée.", 'info');
             render();
        }
    });
};
...
export const leaveLobby = async () => {
    ...
    if (state.activeHeistLobby.host_id === state.activeCharacter.id) {
        await state.supabase.from('heist_lobbies').delete().eq('id', state.activeHeistLobby.id);
        ui.showToast('Équipe dissoute.', 'info');
    } else {
