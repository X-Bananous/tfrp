...
export const ServicesView = () => {
    ...
    // === 3. DOSSIER DETAIL PAGE ===
        ...
        const points = c.driver_license_points !== undefined ? c.driver_license_points : 0; 
        const hasLicenseItem = c.has_physical_license;
        
        let licenseContent = '';
        
        if (!hasLicenseItem) {
            licenseContent = `
                <div class="bg-red-900/20 border border-red-500/30 p-6 rounded-xl text-center">
                    <i data-lucide="file-warning" class="w-12 h-12 text-red-500 mx-auto mb-3"></i>
                    <h3 class="text-lg font-bold text-white mb-1">Non Titulaire</h3>
                    <p class="text-xs text-red-300 uppercase tracking-widest font-bold">Aucun permis physique trouvé</p>
                </div>
            `;
        } else if (points <= 0) {
            licenseContent = `
                <div class="bg-orange-900/20 border border-orange-500/30 p-6 rounded-xl text-center">
                    <i data-lucide="ban" class="w-12 h-12 text-orange-500 mx-auto mb-3"></i>
                    <h3 class="text-lg font-bold text-white mb-1">Permis Suspendu</h3>
                    <p class="text-xs text-orange-300 uppercase tracking-widest font-bold">Solde nul (0 points)</p>
                </div>
            `;
        } else {
            // VALID LICENSE
