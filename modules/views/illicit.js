...
export const HEIST_DATA = [
    { id: 'car_theft', name: 'Vol de Voiture', min: 5000, max: 15000, time: 300, rate: 100, icon: 'car', requiresValidation: false, requiresGang: true, risk: 1, teamMin: 1, teamMax: 2, requiresLocation: false },
    { id: 'atm', name: 'Braquage ATM', min: 1000, max: 4000, time: 90, rate: 100, icon: 'credit-card', requiresValidation: false, requiresGang: false, risk: 2, teamMin: 1, teamMax: 3, requiresLocation: true },
    { id: 'house', name: 'Cambriolage Maison', min: 2000, max: 8000, time: 60, rate: 100, icon: 'home', requiresValidation: false, requiresGang: false, risk: 1, teamMin: 3, teamMax: 5, requiresLocation: true },
    { id: 'gas', name: 'Station Service', min: 3000, max: 10000, time: 105, rate: 100, icon: 'fuel', requiresValidation: false, requiresGang: false, risk: 2, teamMin: 2, teamMax: 6, requiresLocation: true },
    { id: 'truck', name: 'Fourgon Blindé', min: 150000, max: 300000, time: 900, rate: 15, icon: 'truck', requiresValidation: true, requiresGang: true, risk: 4, teamMin: 5, teamMax: 10, requiresLocation: false },
    { id: 'jewelry', name: 'Bijouterie', min: 350000, max: 550000, time: 1020, rate: 10, icon: 'gem', requiresValidation: true, requiresGang: true, risk: 5, teamMin: 2, teamMax: 9, requiresLocation: false },
    { id: 'bank', name: 'Banque Centrale', min: 600000, max: 900000, time: 1200, rate: 5, icon: 'landmark', requiresValidation: true, requiresGang: true, risk: 5, teamMin: 7, teamMax: 13, requiresLocation: false }
];
...
export const IllicitView = () => {
    ...
    // 3. LABORATOIRE DE DROGUE (Missing Block Re-added)
        ...
        if (!hasGang || state.activeGang.myStatus !== 'accepted') {
            content = `
                <div class="flex flex-col items-center justify-center h-full p-10 text-center animate-fade-in">
                    ...
                    <p class="text-gray-400 max-w-sm mx-auto">Vous devez être membre actif d'un gang pour accéder aux laboratoires de production.</p>
                </div>
            `;
        } else {
            const lab = state.drugLab;
            
            // SECURITY CHECK: IF LAB DATA IS NULL (API FAILURE), SHOW LOADER
            if (!lab) {
                content = '<div class="p-8 text-center text-gray-500"><div class="loader-spinner mb-4 mx-auto"></div>Synchronisation du labo...</div>';
            } else if (!lab.has_building) {
                content = `
                    <div class="max-w-2xl mx-auto text-center pt-20">
                        <div class="w-32 h-32 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-8 text-emerald-500 border border-emerald-500/20">
                            <i data-lucide="warehouse" class="w-16 h-16"></i>
                        </div>
                        <h2 class="text-3xl font-bold text-white mb-4">Acquisition Immobilière</h2>
                        <p class="text-gray-400 mb-8 leading-relaxed">
                            Votre gang a besoin d'un local sécurisé pour installer le matériel de production.
                            Ce bâtiment servira de base pour vos opérations chimiques.
                        </p>
                        <button onclick="actions.buyLabComponent('building', 50000)" class="glass-btn px-8 py-4 rounded-xl text-lg font-bold bg-emerald-600 hover:bg-emerald-500 shadow-lg shadow-emerald-900/20">
                            Acheter Local ($50,000)
                        </button>
                        <p class="text-xs text-gray-500 mt-4">Paiement en liquide uniquement.</p>
                    </div>
                `;
            } else if (!lab.has_equipment) {
                content = `
                    <div class="max-w-2xl mx-auto text-center pt-20">
                        <div class="w-32 h-32 bg-blue-500/10 rounded-full flex items-center justify-center mx-auto mb-8 text-blue-500 border border-blue-500/20">
                            <i data-lucide="flask-conical" class="w-16 h-16"></i>
                        </div>
                        <h2 class="text-3xl font-bold text-white mb-4">Équipement de Laboratoire</h2>
                        <p class="text-gray-400 mb-8 leading-relaxed">
                            Le local est vide. Vous devez acheter le matériel nécessaire (distillateurs, lampes UV, systèmes de ventilation) pour commencer la production.
                        </p>
                        <button onclick="actions.buyLabComponent('equipment', 25000)" class="glass-btn px-8 py-4 rounded-xl text-lg font-bold bg-blue-600 hover:bg-blue-500 shadow-lg shadow-blue-900/20">
                            Acheter Matériel ($25,000)
                        </button>
                    </div>
                `;
            } else {
                ...
                content = `
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full min-h-0">
                        ...
                        <div class="space-y-6 overflow-y-auto custom-scrollbar pr-2">
                            <!-- STOCK CARDS -->
                            <div class="grid grid-cols-2 gap-4">
                                <div class="glass-panel p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                                    <div class="text-xs text-gray-500 uppercase font-bold mb-2">Cocaïne (Brute)</div>
                                    <div class="text-2xl font-mono font-bold text-white">${lab.stock_coke_raw || 0}g</div>
                                </div>
                                <div class="glass-panel p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                                    <div class="text-xs text-gray-500 uppercase font-bold mb-2">Cocaïne (Pure)</div>
                                    <div class="text-2xl font-mono font-bold text-emerald-400">${lab.stock_coke_pure || 0}g</div>
                                </div>
                                <div class="glass-panel p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                                    <div class="text-xs text-gray-500 uppercase font-bold mb-2">Weed (Séchage)</div>
                                    <div class="text-2xl font-mono font-bold text-white">${lab.stock_weed_raw || 0}g</div>
                                </div>
                                <div class="glass-panel p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                                    <div class="text-xs text-gray-500 uppercase font-bold mb-2">Weed (Prête)</div>
                                    <div class="text-2xl font-mono font-bold text-emerald-400">${lab.stock_weed_pure || 0}g</div>
                                </div>
                            </div>

                            <!-- ACTIONS FORM -->
                            <div class="glass-panel p-6 rounded-2xl border border-white/5">
                                <h3 class="font-bold text-white mb-4">Opérations</h3>
                                ${isWorking ? `
                                    <div class="p-6 bg-black/40 rounded-xl text-center border border-emerald-500/30">
                                        <div class="loader-spinner mb-4 mx-auto border-emerald-500"></div>
                                        <div class="text-emerald-400 font-bold animate-pulse uppercase tracking-widest mb-1">Production en cours</div>
                                        <div class="text-sm text-gray-400">${DRUG_DATA[batch.type].name} • ${batch.stage}</div>
                                        <div class="font-mono text-2xl font-bold text-white mt-2" id="drug-timer-display">00:00</div>
                                    </div>
                                ` : `
                                    <form onsubmit="return false;" class="space-y-4">
                                        <div class="grid grid-cols-2 gap-4">
                                            <label class="cursor-pointer">
                                                ...
                                                <div class="p-3 rounded-xl bg-white/5 border border-white/5 peer-checked:bg-white peer-checked:text-black hover:bg-white/10 text-center text-sm font-bold transition-all">
                                                    Cocaïne
                                                </div>
                                            </label>
                                            <label class="cursor-pointer">
                                                <input type="radio" name="drug_type" value="weed" class="peer sr-only">
                                                <div class="p-3 rounded-xl bg-white/5 border border-white/5 peer-checked:bg-emerald-500 peer-checked:text-white hover:bg-white/10 text-center text-sm font-bold transition-all">
                                                    Cannabis
                                                </div>
                                            </label>
                                        </div>
                                        ...
                                        <div>
                                            <label class="text-xs text-gray-500 uppercase font-bold mb-2 block">Quantité (Grammes)</label>
                                            <select name="amount" class="glass-input w-full p-3 rounded-xl bg-black/40">
                                                <option value="100">100g (Petite prod)</option>
                                                <option value="500">500g (Moyenne prod)</option>
                                                <option value="1000">1kg (Grosse prod)</option>
                                            </select>
                                        </div>

                                        <div class="grid grid-cols-1 gap-3">
                                            <button onclick="actions.startDrugAction('harvest', event)" class="glass-btn-secondary w-full py-3 rounded-xl font-bold text-sm hover:bg-white/10 flex items-center justify-center gap-2">
                                                <i data-lucide="sprout" class="w-4 h-4"></i> Récolter / Importer
                                            </button>
                                            <button onclick="actions.startDrugAction('process', event)" class="glass-btn-secondary w-full py-3 rounded-xl font-bold text-sm hover:bg-blue-500/20 hover:text-blue-300 border-blue-500/20 flex items-center justify-center gap-2">
                                                <i data-lucide="flask-conical" class="w-4 h-4"></i> Traiter / Cuisiner
                                            </button>
                                            <button onclick="actions.startDrugAction('sell', event)" class="glass-btn w-full py-3 rounded-xl font-bold text-sm bg-emerald-600 hover:bg-emerald-500 shadow-lg shadow-emerald-900/20 flex items-center justify-center gap-2">
                                                <i data-lucide="dollar-sign" class="w-4 h-4"></i> Vendre au Deal
                                            </button>
                                        </div>
                                    </form>
                                `}
                        ...
                        <!-- RIGHT: INFO / RECIPES -->
                        <div class="glass-panel p-6 rounded-2xl border border-white/5 h-fit">
                            <h3 class="font-bold text-white mb-4 flex items-center gap-2"><i data-lucide="book-open" class="w-5 h-5 text-gray-400"></i> Guide de Production</h3>
                            
                            <div class="space-y-6 text-sm text-gray-400 leading-relaxed">
                                <div>
                                    <h4 class="text-white font-bold mb-1">1. Récolte (Harvest)</h4>
                                    <p>Importation de feuilles de coca ou culture de plants de cannabis. Nécessite du temps. <span class="text-orange-400 text-xs block mt-1">⚠️ Limité à 1x par semaine.</span></p>
                                </div>
                                <div>
                                    <h4 class="text-white font-bold mb-1">2. Traitement (Process)</h4>
                                    <p>Transformation de la matière brute en produit pur prêt à la vente. Consomme le stock brut.</p>
                                </div>
                                <div>
                                    <h4 class="text-white font-bold mb-1">3. Vente (Sell)</h4>
                                    <p>Distribution via les "Go Fast". L'argent est versé directement en liquide (non-bancaire). Risque élevé d'interception police.</p>
                                </div>
                                
                                <div class="p-3 bg-white/5 rounded-xl border border-white/5 mt-4">
                                    <div class="flex justify-between text-xs mb-1"><span>Prix Coke (g)</span><span class="text-white font-bold">$${DRUG_DATA.coke.pricePerG}</span></div>
                                    <div class="flex justify-between text-xs"><span>Prix Weed (g)</span><span class="text-white font-bold">$${DRUG_DATA.weed.pricePerG}</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    // 6. BRAQUAGES
        ...
        // ... (Code identique au précédent pour heists)
        if (!state.activeGameSession) {
            content = `
                <div class="flex flex-col items-center justify-center h-full p-10 text-center animate-fade-in">
