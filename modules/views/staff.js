...
export const StaffView = () => {
    ...
    if (state.activeStaffTab === 'applications' && hasPermission('can_approve_characters')) {
                ...
                <div class="glass-panel p-6 rounded-2xl mb-6 border border-red-500/20 bg-red-900/10">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            ` : `
                                <form onsubmit="actions.activateMaintenance(event)" class="flex gap-2 items-end">
                                    <div>
                                        <label class="text-[9px] text-gray-500 uppercase font-bold block mb-1">Durée (min)</label>
                                        <input type="number" name="duration" value="30" class="glass-input w-16 p-1.5 rounded text-xs bg-black/40">
                                    </div>
                                    <div>
                                        <label class="text-[9px] text-gray-500 uppercase font-bold block mb-1">Raison</label>
                                        <input type="text" name="reason" placeholder="Mise à jour..." class="glass-input w-32 p-1.5 rounded text-xs bg-black/40">
                                    </div>
                                    <button type="submit" class="glass-btn px-4 py-1.5 rounded-lg text-xs font-bold bg-white text-black hover:bg-gray-200 h-[30px]">Activer</button>
                                </form>
                            `}
                        </div>
                    </div>
        ...
        `;
         ...
         const killLogs = state.erlcData.killLogs || [];
         ...
         content = `
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <!-- MOD CALLS -->
                 <div class="glass-panel p-6 rounded-2xl md:col-span-2 lg:col-span-3 border border-white/5">
                    <h3 class="font-bold text-white mb-4 flex items-center gap-2">
                        <i data-lucide="radio" class="w-5 h-5 text-blue-400"></i>
                        Appels Modération
                    </h3>
                    <div class="flex flex-wrap gap-4 max-h-[300px] overflow-y-auto custom-scrollbar">
                ...
                <div class="glass-panel p-6 rounded-2xl md:col-span-2 lg:col-span-3 border border-white/5">
                    <h3 class="font-bold text-white mb-4 flex items-center gap-2">
                        ...
                        Journal des Meurtres (Kill Logs)
                    </h3>
                    <div class="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar">
                        ${killLogs.length > 0 ? 
                            killLogs.map(k => {
                                const victim = k.Killed || k.Victim || 'Inconnu';
                                // Enhanced Weapon Check (Case Insensitive fallbacks)
                                const tool = k.Tool || k.tool || k.Weapon || k.weapon || 'Inconnu';
                                return `
                                <div class="bg-white/5 p-2 rounded-lg border border-white/5 flex items-center justify-between text-xs hover:bg-white/10 transition-colors">
                                    <div class="flex items-center gap-2">
                                        <span class="font-bold text-red-400">${k.Killer || 'Inconnu'}</span>
                                        <span class="text-gray-600"><i data-lucide="sword" class="w-3 h-3"></i></span>
                                        <span class="font-bold text-white">${victim}</span>
                                    </div>
                                    <div class="flex items-center gap-4">
                                        <span class="text-gray-400 italic bg-black/20 px-2 py-0.5 rounded">${tool}</span>
                                        <span class="text-gray-600 font-mono">${k.Timestamp ? new Date(k.Timestamp * 1000).toLocaleTimeString() : ''}</span>
                                    </div>
                                </div>
                                `;
                            }).join('') 
                        : '<div class="text-center text-gray-500 italic py-4">Aucun meurtre enregistré récemment.</div>'}
                    </div>
                </div>
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
                        ...
                        <div class="relative w-full md:w-64">
                            ...
                            <input type="text" 
                                ...
                                class="glass-input pl-10 w-full py-2 rounded-lg text-xs bg-black/40">
                        </div>
                    </div>
    ...
    return `
        ...
        <div class="h-full flex flex-col bg-[#050505] overflow-hidden animate-fade-in relative">
            ...
            <!-- FIXED HEADER -->
            <div class="px-6 pb-4 pt-4 flex flex-col gap-4 border-b border-white/5 shrink-0 z-10 relative bg-[#050505]">
                ...
                <div class="flex gap-2 bg-white/5 p-1 rounded-xl overflow-x-auto max-w-full no-scrollbar">
                    ${hasPermission('can_approve_characters') ? `
                        <button onclick="actions.setStaffTab('applications')" class="px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${state.activeStaffTab === 'applications' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}">
                            <i data-lucide="file-check" class="w-4 h-4"></i> Candidatures
                        </button>
                    ` : ''}
                    
                    <button onclick="actions.setStaffTab('database')" class="px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${state.activeStaffTab === 'database' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}">
                        <i data-lucide="database" class="w-4 h-4"></i> Base de Données
                    </button>
                    
                    ${hasPermission('can_manage_economy') || hasPermission('can_manage_illegal') ? `
                        <button onclick="actions.setStaffTab('economy')" class="px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${state.activeStaffTab === 'economy' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}">
                            <i data-lucide="coins" class="w-4 h-4"></i> Économie
                        </button>
                    ` : ''}
                    ${hasPermission('can_manage_illegal') ? `
                        <button onclick="actions.setStaffTab('illegal')" class="px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${state.activeStaffTab === 'illegal' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}">
                            <i data-lucide="skull" class="w-4 h-4"></i> Illégal
                        </button>
                    ` : ''}
                    ${hasPermission('can_manage_enterprises') ? `
                        <button onclick="actions.setStaffTab('enterprise')" class="px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${state.activeStaffTab === 'enterprise' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}">
                            <i data-lucide="building-2" class="w-4 h-4"></i> Entreprises
                        </button>
                    ` : ''}
                    ${hasPermission('can_manage_staff') ? `
                        <button onclick="actions.setStaffTab('permissions')" class="px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${state.activeStaffTab === 'permissions' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}">
                            <i data-lucide="shield" class="w-4 h-4"></i> Permissions
                        </button>
                    ` : ''}
                    
                     <button onclick="actions.setStaffTab('sessions')" class="px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${state.activeStaffTab === 'sessions' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}">
                        <i data-lucide="play-circle" class="w-4 h-4"></i> Sessions
                    </button>
                    
                     <button onclick="actions.setStaffTab('logs')" class="px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${state.activeStaffTab === 'logs' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}">
                        <i data-lucide="file-text" class="w-4 h-4"></i> Logs & ERLC
                    </button>
                </div>
            </div>

            <!-- SCROLLABLE CONTENT WITH FIXED PADDING AND WIDTH CONSTRAINTS -->
            <div class="flex-1 p-4 md:p-6 overflow-y-auto custom-scrollbar relative min-h-0 w-full">
