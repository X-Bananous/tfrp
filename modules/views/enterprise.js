            ...
            <div class="flex flex-col h-full overflow-hidden animate-fade-in">
                ...
                <div class="flex-1 overflow-y-auto custom-scrollbar pr-2">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        ${ents.map(ent => {
                            // Check if already a member/applicant
                    ...
                    <div class="flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-1">
                        ...
                        <div class="bg-white/5 border border-white/5 rounded-xl p-4">
                            ...
                            <div class="text-[10px] text-orange-300 mb-3 bg-orange-500/10 p-2 rounded border border-orange-500/20">
                                <i data-lucide="info" class="w-3 h-3 inline mr-1"></i> Taxe de mise en rayon : 5% du prix total.
                            </div>
                            <form onsubmit="actions.addItemToMarket(event)" class="space-y-3">
                                ...
                                <button type="submit" class="glass-btn w-full py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500">
                                    <i data-lucide="plus" class="w-3 h-3"></i> Ajouter Produit
                                </button>
                            </form>
                        </div>
                                ...
                                <table class="w-full text-left text-xs">
                                    ...
                                    <tbody class="divide-y divide-white/5">
                                        ${ent.items.map(i => {
                                            ...
                                            const taxAmount = Math.ceil(i.price * i.quantity * 0.05);

                                            return `
                                            <tr class="hover:bg-white/5 transition-colors group">
                                                <td class="p-3">
                                                    <div class="font-medium text-white ${i.is_hidden ? 'opacity-50' : ''}">${i.name}</div>
                                                    ${isPending ? '<span class="text-[9px] text-orange-400 uppercase font-bold">Validation</span>' : 
                                                      isAwaitingTax ? `<button onclick="actions.payItemTax('${i.id}')" class="text-[9px] bg-red-600 hover:bg-red-500 text-white px-2 py-0.5 rounded uppercase font-bold mt-1">Payer Taxe ($${taxAmount})</button>` : ''}
                                                </td>
                                                ...
                                                <td class="p-3 text-right flex justify-end gap-1">
                                                    ${!isAwaitingTax ? `<button onclick="actions.restockItem('${i.id}')" class="p-1.5 bg-blue-500/10 text-blue-400 rounded hover:bg-blue-500/20"><i data-lucide="plus" class="w-3 h-3"></i></button>` : ''}
