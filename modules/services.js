...
export const createNotification = async (title, message, type = 'info', isPinned = false) => {
    if (!state.supabase) return;
    await state.supabase.from('notifications').insert({
        title, message, type, is_pinned: isPinned
    });
};
...
/**
 * SHARED GANG LAB FETCH (READ ONLY)
 * Évite les mises à jour en boucle.
 */
...
/**
 * LOGIQUE DE COMPLÉTION DU BATCH (SÉPARÉE)
 * Appelé via Rafrachissement manuel ou Polling, pas par le listener Temps Réel.
 */
export const checkAndCompleteDrugBatch = async (gangId) => {
    if (!gangId) return;
    const { data: lab = null } = await state.supabase.from('drug_labs').select('*').eq('gang_id', gangId).maybeSingle();
    ...
    if (lab && lab.current_batch) {
        ...
        if (new Date(lab.current_batch.end_time).getTime() <= now) {
            ...
            if (stage === 'harvest') {
                ...
                showToast(`Vente terminée (Gang) : +$${reward} dans le coffre du gang.`, 'success');
            }

            await state.supabase.from('drug_labs').update(updates).eq('id', lab.id);
            await fetchDrugLab(gangId);
        }
    }
};
