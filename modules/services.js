...
export const setupRealtimeListener = () => {
    ...
    channel
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'drug_labs' }, async () => {
            if (state.activeHubPanel === 'illicit' && state.activeGang) {
...
export const updateMaintenance = async (isActive, durationMinutes, reason) => {
    ...
    let endTime = null;
    if (isActive && durationMinutes) { endTime = new Date(Date.now() + durationMinutes * 60000).toISOString(); }
    const { error } = await state.supabase.from('maintenance').update({ is_active: isActive, end_time: endTime, reason: reason, activated_by: isActive ? state.user.username : null }).eq('id', 1);
    if (error) { showToast("Erreur mise à jour maintenance.", 'error'); return false; }
    await fetchMaintenanceStatus();
    return true;
};
...
export const stopSession = async () => {
    ...
    const { error } = await state.supabase.from('game_sessions').update({ end_time: new Date(), end_player_count: endCount, peak_player_count: peak, mod_calls_count: modCallsCount, bans_count: bansCount, status: 'finished' }).eq('id', state.activeGameSession.id);
    if(!error) { state.activeGameSession = null; await state.supabase.from('profiles').update({ is_on_duty: false }).neq('id', '0'); await fetchOnDutyStaff(); showToast("Session fermée. Staff mis hors service.", 'info'); await fetchSessionHistory(); } else { showToast("Erreur fermeture session.", 'error'); }
};
...
export const updateGangBalance = async (gangId, newBalance) => { const { error } = await state.supabase.from('gangs').update({ balance: newBalance }).eq('id', gangId); if(error) console.error("Balance update failed", error); };
export const fetchBounties = async () => { const { data: data } = await state.supabase.from('bounties').select('*, creator:characters!bounties_creator_id_fkey(first_name, last_name)').order('amount', { ascending: false }); state.bounties = data || []; };
...
export const fetchEnterprises = async () => { const { data } = await state.supabase.from('enterprises').select('*, leader:characters!enterprises_leader_id_fkey(first_name, last_name)'); state.enterprises = data || []; };
...
export const fetchEnterpriseMarket = async () => {
    ...
    if (autoEcole) {
        const { data: items } = await state.supabase.from('enterprise_items').select('name').eq('enterprise_id', autoEcole.id);
        const hasLicense = items?.some(i => i.name === 'Permis de conduire');
        const hasStage = items?.some(i => i.name === 'Stage de récupération de points');
        const newItems = [];
        if (!hasLicense) { newItems.push({ enterprise_id: autoEcole.id, name: 'Permis de conduire', price: 1450, quantity: 999999, payment_type: 'both', status: 'approved', is_hidden: false, description: 'Prendre RDV pour passer le Permis officiel.' }); }
        if (!hasStage) { newItems.push({ enterprise_id: autoEcole.id, name: 'Stage de récupération de points', price: 200, quantity: 999999, payment_type: 'both', status: 'approved', is_hidden: false, description: 'Prendre RDV pour un stage (+3 points)' }); }
        if (newItems.length > 0) { await state.supabase.from('enterprise_items').insert(newItems); }
    }
...
export const fetchTopSellers = async () => {
    ...
    if (state.topSellers.length < 3) {
        ...
        if (state.topSellers.length < 3) {
            const others = state.enterpriseMarket.filter(i => !state.topSellers.includes(i));
            others.sort(() => Math.random() - 0.5);
            for (const item of others) {
                if (state.topSellers.length >= 3) break;
                state.topSellers.push({...item, is_random_trend: true});
            }
        }
    }
};
...
export const fetchEnterpriseDetails = async (entId) => {
    ...
    if (ent.name === 'L.A. Auto School') {
         const { data: itemsCheck } = await state.supabase.from('enterprise_items').select('name').eq('enterprise_id', entId);
         const hasLicense = itemsCheck?.some(i => i.name === 'Permis de conduire');
         const hasStage = itemsCheck?.some(i => i.name === 'Stage de récupération de points');
         const newItems = [];
         if (!hasLicense) newItems.push({ enterprise_id: entId, name: 'Permis de conduire', price: 1450, quantity: 999999, payment_type: 'both', status: 'approved', is_hidden: false, description: 'Prendre RDV pour passer le Permis officiel.' });
         if (!hasStage) newItems.push({ enterprise_id: entId, name: 'Stage de récupération de points', price: 200, quantity: 999999, payment_type: 'both', status: 'approved', is_hidden: false, description: 'Prendre RDV pour un stage (+3 points)' });
         if (newItems.length > 0) await state.supabase.from('enterprise_items').insert(newItems);
    }
    ...
    const { data: invoices } = await state.supabase.from('invoices').select('*, characters:buyer_id(first_name, last_name)').eq('enterprise_id', entId).order('created_at', { ascending: false }).limit(20);
    const { data: appointments } = await state.supabase.from('enterprise_appointments')
        .select(`*, characters:client_id (first_name, last_name, user_id)`)
        .eq('enterprise_id', entId)
        .eq('status', 'pending');
    let enhancedAppointments = [];
    if (appointments && appointments.length > 0) {
        const userIds = appointments.map(a => a.characters.user_id);
        const { data: profiles } = await state.supabase.from('profiles').select('id, username').in('id', userIds);
        enhancedAppointments = appointments.map(a => {
    ...
    state.activeEnterpriseManagement = { 
        ...ent, members: members || [], items: items || [], promos: promos || [],
        invoices: invoices || [], appointments: enhancedAppointments || [],
        circulation: circulation, myRank: myMember ? myMember.rank : null 
    };
};
export const createEnterprisePromo = async (promoData) => { const { error } = await state.supabase.from('enterprise_promos').insert(promoData); if(error) showToast("Erreur création promo.", "error"); else showToast("Code promo créé.", "success"); };
...
export const adminCreateCharacter = async (charData) => { const { error } = await state.supabase.from('characters').insert([charData]); return !error; };
...
export const claimAdventReward = async (targetDay) => {
    ...
    if (targetDay === 16) {
        try {
            const { error } = await state.supabase.from('inventory').insert({ character_id: charId, name: "Anniversaire de la patate 2025", quantity: 1, estimated_value: 5000 });
...
export const fetchBankData = async (charId) => {
    let { data: bank, error } = await state.supabase.from('bank_accounts').select('*').eq('character_id', charId).maybeSingle(); 
    if (!bank) { const { data: newBank } = await state.supabase.from('bank_accounts').insert([{ character_id: charId, bank_balance: 5000, cash_balance: 500 }]).select().single(); bank = newBank; }
...
export const fetchInventory = async (charId) => {
    if (!state.supabase) return;
    await fetchBankData(charId);
    const { data: items, error } = await state.supabase.from('inventory').select('*').eq('character_id', charId);
    state.inventory = items || [];
    const requiredVirtualItems = [ { id: 'virtual-id-card', name: "Carte d'Identité", icon: 'id-card' }, { id: 'virtual-credit-card', name: "Carte Bancaire", icon: 'credit-card' }];
    requiredVirtualItems.forEach(vItem => { const exists = state.inventory.some(i => i.name === vItem.name); if (!exists) { state.inventory.unshift({ id: vItem.id, name: vItem.name, quantity: 1, estimated_value: 0, is_virtual: true, icon: vItem.icon }); } });
...
/**
 * SHARED GANG LAB FETCH
 * Synchronisé par gang_id
 */
export const fetchDrugLab = async (gangId) => {
    ...
    if (lab && lab.current_batch) {
        const now = Date.now();
        if (new Date(lab.current_batch.end_time).getTime() <= now) {
            const batch = lab.current_batch;
            const updates = { current_batch: null };
            const { type, stage, amount } = batch;

            if (stage === 'harvest') {
                const key = type === 'coke' ? 'stock_coke_raw' : 'stock_weed_raw';
                updates[key] = (lab[key] || 0) + amount;
                showToast(`Récolte terminée (Gang) : +${amount}g`, 'success');
            }
            else if (stage === 'process') {
                const key = type === 'coke' ? 'stock_coke_pure' : 'stock_weed_pure';
                updates[key] = (lab[key] || 0) + amount;
                showToast(`Traitement terminé (Gang) : +${amount}g`, 'success');
            }
            else if (stage === 'sell') {
                ...
                if(gang) {
                    await state.supabase.from('gangs').update({ balance: (gang.balance || 0) + reward }).eq('id', gangId);
                }
                showToast(`Vente terminée (Gang) : +$${reward} dans le coffre fort du gang.`, 'success');
            }

            await state.supabase.from('drug_labs').update(updates).eq('id', lab.id);
            lab = { ...lab, ...updates, current_batch: null };
        }
    }
    state.drugLab = lab;
};
...
export const startHeistSync = async (durationSeconds) => { if(!state.activeHeistLobby) return; const now = Date.now(); const endTime = now + (durationSeconds * 1000); const { error } = await state.supabase.from('heist_lobbies').update({ status: 'active', start_time: now, end_time: endTime }).eq('id', state.activeHeistLobby.id); if(error) showToast("Erreur lancement", 'error'); await fetchActiveHeistLobby(state.activeCharacter.id); };
